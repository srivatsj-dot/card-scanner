...
export function scanSystemPrompt(settings: Settings, hasImage = true): string {
  ...
  return (
    ...
    `\n\nWhen you analyze the card, fill every field you can verify. TRADE IDEAS MUST BE REAL AND VALUE-ANCHORED — this is where bad output shows most, so hold a high bar:` +
    `\n• Anchor everything to THIS card's estimated value. An "upgrade" (recommendedTrades) is a SPECIFIC, REAL card that is a genuine step UP — clearly MORE valuable and more desirable than this one (roughly 1.5×–5× its value), that a collector of this card would realistically chase. A generic base card or insert worth a dollar is NOT an upgrade for a valuable card; never suggest one.` +
    `\n• A "fair swap" (similarValueTargets) is a SPECIFIC, REAL card of genuinely similar market value (within ~20%) that the other side would actually accept 1-for-1.` +
    `\n• Name real, verifiable cards — actual player + set + year + (parallel/insert) that exist. Do NOT invent card numbers; leave the number out unless you are sure of it. Do NOT pad the list with near-identical generic "base insert" entries for a string of different players — that is filler and is wrong. Prefer FEWER, accurate, well-differentiated ideas (even just 1-2) over 4 vague ones.` +
    `\n• Make each value estimate realistic for that specific card and consistent with the upgrade/fair-swap role. If you cannot think of a genuinely accurate comparable, return an empty list rather than guessing.` +
    `\nSuggest up to 4 upgrades and up to 4 fair swaps, both respecting the collector's filters. For "playerOutlook.trend", use exactly one of: rising, stable, declining, unknown.` +
