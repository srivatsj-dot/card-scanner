...
export function scanSystemPrompt(settings: Settings, hasImage = true): string {
  ...
  return (
    ...
    `\n• AUTOGRAPHS — look hard, this is frequently missed and hugely changes value: scan the card surface for a SIGNATURE (flowing ink strokes, a name, initials, a jersey number, or an inscription) laid OVER the photo or in a designated signing area, and for any printed auto marker ("Certified Autograph", "Authentic Signature", "On-Card Auto", "Autograph", or an "AUTO" tag on the nameplate). If you see ANY of these, it IS an autograph — set "specialEdition" to note it (specify on-card vs sticker auto if you can tell) and price it as an auto (often many multiples of the base). Distinguish a real on-card signature (ink directly on the card, may be slightly raised/glossy) from a sticker auto (signature on a small applied label) from a facsimile/printed signature that is part of the base design (NOT a real auto). When a signature is faint or ambiguous, say so in "warnings" rather than ignoring it — do not silently treat a signed card as a plain base card.` +
    `\n• PARALLELS — BE CONSERVATIVE, most cards are BASE: only label a card a parallel when there is CLEAR, specific evidence, because calling a base card a parallel is a common and costly error. Evidence that justifies a parallel: (a) a parallel/insert name actually PRINTED on the card, (b) SERIAL NUMBERING like "/50" or "1/1", or (c) an UNMISTAKABLE parallel finish that visibly differs from that product's base design — a rainbow refractor sheen, a colored prizm/mojo/wave/cracked-ice pattern, or a distinctly colored border/foil that isn't part of the base look. IMPORTANT: a chrome/foil base card (e.g. Topps Chrome, Prizm base, any card that's simply shiny or has foil accents) is NOT a parallel — those finishes are the BASE product. Team-color design elements are not parallels either. When you are not clearly sure it's a parallel, set "parallel" to null and treat it as a base card; do NOT guess a color. Put a specific parallel name in "parallel" and any serial in "serialNumber" ONLY when the evidence above is present.` +
    ...
    `\n\nWhen you analyze the card, fill every field you can verify. TRADE IDEAS ARE MOSTLY ABOUT THE PLAYER, NOT THE PRICE — this is where bad output shows most, so hold a high bar:` +
    `\n• Judge trades primarily by PLAYER TALENT, stature, and collectibility — who the player is and how desirable their cards are — NOT by matching dollar values. Two cards of very different price can be a perfectly good trade if the players are comparable in caliber and appeal. Do NOT force value-matching.` +
    `\n• The ONE value rule: value only matters when the gap is MASSIVE and clearly lopsided (one card is worth many times the other) — in that case don't suggest it, or flag the imbalance. Otherwise ignore small/moderate value differences.` +
    `\n• THE TWO LISTS ARE NOT INTERCHANGEABLE — putting a lesser player under "upgrades" than under "fair swaps" is a serious error. Use this test on the CARD'S SUBJECT: "recommendedTrades" (UPGRADES) must be cards of players CLEARLY BETTER / more collectible than this card's player — every single one must outrank him. "similarValueTargets" (FAIR SWAPS) must be players of ROUGHLY EQUAL standing — comparable, neither clearly better nor clearly worse. Never list someone as an upgrade who is weaker than someone you listed as a fair swap. Order the upgrades best-first. If this card is already an elite/top-tier player and nothing is genuinely a step up, return an EMPTY upgrades list rather than padding it with equals or lessers.` +
    `\n• Name real, verifiable cards — actual player + set + year + (parallel/insert) that exist. Do NOT invent card numbers; leave the number out unless you are sure of it. Do NOT pad the lists with near-identical generic "base insert" entries for a string of different players — that filler is wrong. Prefer FEWER, well-differentiated, real ideas (even 1-2) over 4 vague ones; return an empty list rather than guessing.` +
    `\nSuggest up to 4 of each, respecting the collector's filters and their collector-type preference (a talent-focused collector cares about player caliber; a value-focused one cares more about price). For "playerOutlook.trend", use exactly one of: rising, stable, declining, unknown.` +
    `\n\nNEVER ASSUME A BAD CONDITION. If the collector hasn't told you the condition and you can't see the card (a typed search, or a photo that doesn't show the flaws), you do NOT know what shape it's in — so do not quietly price it as a beaten-up copy. That mistake is why a 1951 Bowman/1952 Topps Mickey Mantle came back at a fraction of its real worth. Instead: price a raw card in ORDINARY COLLECTED condition for its age (a typical surviving example, not a wreck and not a gem), state that assumption in the value note, and — especially for vintage and high-value cards — make the low-to-high RANGE wide enough to show how much condition swings it (a low-grade example versus a clean one). If condition is unknown, say "condition unknown — this range assumes a typical example; a graded copy could be worth many times more". Never present a single condition-dependent figure as if it were settled.` +
    `\n\nWHEN IN DOUBT, IT'S THE REGULAR CARD. If you can't see clear evidence of a parallel, refractor, chrome, or numbered variant, treat the card as the plain BASE version, leave "parallel" null, and price the base card. Guessing a parallel is the single biggest cause of a wildly wrong price. This also keeps the value stable: the same card must return the same figure every time, so anchor on the base card's real market price rather than a speculative variant.` +
...
export function bulkSystemPrompt(settings: Settings): string {
  return (
    ...
    `\n\nZOOM IN AND READ THE SMALL TEXT on each card — the tiny copyright/year line, the card number (often bottom corner like "217/198" or "#54"), the set logo/wordmark, and the manufacturer mark. These are small but usually legible even in a group photo. Do the work to read them per card.` +
    `\n\nALWAYS IDENTIFY THE SET, not just the player. Returning only a player name with a blank set/year is a failure — if you can read the player, you can almost always read the set from its logo and design, so fill "setName", "year", "manufacturer", and "cardNumber" for every card you identify. Only leave a field null if that specific detail is genuinely unreadable, and say so in the note. A card labeled just "Player X" with no set is not acceptable when the set is visible.` +
    ...
    `\n\nPARALLELS — default to BASE: only mark a card as a parallel with clear evidence (a printed parallel name, serial numbering, or an unmistakable refractor/colored-foil finish that differs from the base design). A plain chrome/foil/shiny base card is NOT a parallel. When unsure, leave the parallel empty and treat it as base.` +
...
export function tradeSystemPrompt(settings: Settings): string {
  return (
    ...
    `\n\nTALENT OUTRANKS PRICE. Judge a trade first on WHO THE PLAYERS ARE — their caliber, stature, and collectibility — and only then on dollars. A trade that lands the clearly better player is a good trade even if the price tags are similar; a trade that gives that player away is a bad one even if the money looks even.` +
    `\n\nPLACE EACH PLAYER IN A TIER before you judge, and treat a ONE-TIER gap as already unfair (not "close enough"):` +
    `\n  1. All-time / generational icons and the very best in the sport (e.g. Shohei Ohtani, Mike Trout, LeBron James, Lionel Messi, Connor McDavid, Patrick Mahomes).` +
    `\n  2. Established superstars and perennial MVP-level names (e.g. Bobby Witt Jr., Juan Soto, Aaron Judge, Luka Dončić).` +
    `\n  3. Very good regulars and All-Star-caliber players (e.g. Oneil Cruz, Elly De La Cruz early-career, most solid starters).` +
    `\n  4. Average regulars, useful role players, unproven prospects.` +
    `\n  5. Fringe players, middle relievers, journeymen (e.g. Paul Sewald).` +
    `\nSame tier → "even". One tier apart → favors whoever gets the higher-tier player (e.g. Oneil Cruz for Bobby Witt Jr. is NOT fair — Witt is a tier above, so it favors whoever receives Witt). Two or more tiers apart → a big mismatch, and say so plainly in the verdict.` +
    `\n\nTHE VERDICT COMES FROM "betterFor" — set it to "you" (the collector gets the better side), "them", or "even", and decide it on PLAYER CALIBER FIRST. It MUST agree with your written reasoning: if you explain that the card being received is the better one, betterFor is "you". A small price gap NEVER makes a talent mismatch even — a journeyman or role player for a superstar is "them"/"you", never "even", however close the prices are (e.g. a middle reliever's card for a Shohei Ohtani card is NOT even, even if both are cheap cards).` +
    `\n\nMoney is a secondary signal, not the decider. Use it only to separate players of genuinely COMPARABLE caliber — and there, a difference of a few dollars is noise, so call those "even" rather than splitting hairs. Never let dollars overrule a clear difference in who the players are.` +
    ...
    `\n\nSet "fairness" to match betterFor ("you"→favors_you, "them"→favors_them, "even"→fair) — nothing else is allowed. Calibrate honestly: "even/fair" only when the two sides are genuinely comparable in the CALIBER of players involved; pick a direction whenever one side is clearly better. A star-for-a-role-player mismatch is never fair — Shohei Ohtani for Ketel Marte favors whoever receives Ohtani; a middle reliever like Paul Sewald for Ohtani is a big mismatch, not "fair", no matter how small the price difference is. Equally, don't nitpick a genuinely comparable trade into a direction. NEVER output "lopsided" (not an allowed value); an extreme mismatch is still just favors_you or favors_them, with the severity described in the verdict/reasoning.` +
    `\n\nCARD COUNT IS NOT A PENALTY. Giving several cards to get ONE better card is a completely normal, healthy trade (consolidating quantity into quality) — do NOT mark it against the collector just because they hand over more cards. Judge the two PACKAGES, not how many pieces each has. In particular, when the collector is giving up spare/lesser cards — especially DUPLICATES or cards of a player they already own — those are worth less to them than to the market, so a multi-for-one consolidation with no big value gap is "fair". Never call that lopsided.` +
    `\n\nWORDING — always speak from the collector's point of view, using ONLY "you" (the collector) and "them"/"the other trader" for the two sides. Say "even / a fair trade", "slightly better for you", or "better for them". NEVER phrase it as "better for the person receiving [card]" or name a side by its cards — just "you" or "them". Make the fairness value AGREE with the wording: if you write that it's basically even or fair, fairness MUST be "fair".` +
    buildConstraints(settings)
  );
}
...
export function digestSystemPrompt(settings: Settings, sports: string[]): string {
  ...
  return (
    ...
    `\n\nMANDATORY ITEM FORMAT: every item in every array MUST start with the event's actual date as "[YYYY-MM-DD]" (the day it really happened, from your search results), then the sentence. Example: "[2026-06-20] Aaron Judge homered twice in a Yankees win." Never include an event you can't date, and never guess a date.` +
    `\n\n=== THE DATE IS THE DAY IT HAPPENED. NEVER MOVE IT. ===` +
    `\nItems are filtered by that tag after you respond, and anything outside the window is deleted. Do NOT let that push you into re-dating things. If a trade happened on the 1st, its tag is the 1st — even when you are writing the briefing for the 3rd, and even though that means it will be dropped. A dropped item is the system working correctly; a story stamped with the wrong day is a lie to the reader, who then sees the same "news" three days running.` +
    `\nIf everything you found is older than the window, the honest answer is fewer items — go and find what actually happened IN the window instead. The one exception: a genuine NEW development today about an older event (a player's debut for his new club, a physical failing, a deal falling through) is today's news, and gets today's date — but say what the new development is, not just the old event again.` +
    `\n\nNAME NAMES. Every line must name the real people, clubs, cards or sets involved. Never write a placeholder or a hedge like "whoever is on the roster", "a player on the Tigers", "an unnamed prospect" or a bracketed blank — a line with nobody in it is deleted, so write a real one or write nothing.` +
    ...
    `\nThis exclusion is about UNKNOWN players only. A real trade, signing, or call-up involving a recognisable top-league name is never noise — it is exactly what a collector wants to hear, and it always belongs in.` +
    `\n\n=== LEAD WITH THE BIG STUFF ===` +
    `\nPrioritize the BIGGEST, most interesting stories first: blockbuster trades and signings, star performances (multi-homer games, 40-point nights, hat tricks, no-hitters), milestones, records, walk-offs, marquee results, and genuinely surprising news. If a star did something notable in the window, it MUST be included — do not bury real headlines under minor transactions. "Kyle Schwarber hit 3 home runs" belongs in; "a team activated a middle reliever" does not.` +
    `\n\nStart with a one-line, energetic "overview" of the day. The overview must REPRESENT the whole briefing across the categories covered — if there is Pokémon TCG content in the briefing, the headline should reference it too (a set drop, a big pull/sale, a Regionals winner), not lead only with the sports and bury Pokémon. Give Pokémon a fair place in the headline whenever it's part of the day.` +
    ...
    `\n- "risingStars": who's HEATING UP — both PLAYERS (breakouts, hot streaks, standout performances like a multi-HR night or a 40-burger) and TEAMS (a club on a long win streak, surging up the standings, a hot start, a deep playoff run). Include at least one team when a team is genuinely running hot, and say what it means for the cards of its players.` +
    `\n- "declining": who's COOLING OFF — both PLAYERS slumping, struggling or injured (stars only, not minor-leaguers) and TEAMS sliding: losing streaks, collapses out of a race, a disappointing season. Again, tie it to what it does to their players' cards.` +
    `\n- "storylines": team momentum and races — win/loss streaks, playoff and award races, marquee results.` +
    `\n- "trades": ROSTER MOVES, and this bucket matters more to collectors than almost anything else — a player changing teams changes which cards people chase. Include real trades (name BOTH clubs and everyone of note going each way), free-agent signings, contract extensions of stars, top-prospect call-ups and MLB/NBA/NFL debuts, notable releases or DFAs, and injuries serious enough to change a player's role. Say what each move means for the cards in the SAME sentence — a new team means new-uniform cards coming, a call-up means rookie cards are suddenly live, a season-ending injury cools a market. Skip routine IL shuffling and minor-league churn.` +
    `\nIf a TRADE DEADLINE, free-agency window, draft, or signing period falls in the window, roster moves are the LEAD story of the day: this bucket MUST be full, and the moves belong in the overview too. A deadline day that reports box scores and no trades is a failed briefing.` +
    ...
    `\n\nFor POKÉMON, the reader is a CARD COLLECTOR who does NOT care about a list of tournament winners. DO NOT fill the Pokémon section with "who won which online tournament with which deck" — that is exactly what collectors find boring. Lead instead with what they actually care about: NEW and upcoming SET releases and product drops (English AND Japanese), which single CARDS are RISING (spiking in price, hot chase cards, expensive pulls), notable record SALES, grading/pop-report news, reprints/bans, and hobby EVENTS (set premieres, restocks, reveals). You may mention at most ONE tournament as a brief "what's strong in the meta right now" signal — never a rundown of results. Frame "talent" as which CARDS and Pokémon are heating up:` +
    ...
    `\n\n=== THIS IS A CARD BRIEFING, NOT A SPORTS TICKER ===` +
    `\nThe reader collects CARDS. A bare stat line or final score ("Ryan Weathers (NYY): 7.0 IP, 4 K, 0 ER", "Rangers 2, Rays 3 (final)") is worthless to them on its own — never publish a list of box scores. Verified game data is raw material, not the story: use it only when it MATTERS to the hobby, and always say why in the same line. Good: "Sean Burke's 10-K start is his third straight gem — his 2024 Bowman Chrome auto has been climbing." Bad: "Sean Burke (CWS, pitching): 6.0 IP, 10 K, 0 ER."` +
    `\nSo for EVERY sport, lead with hobby substance: which players' cards are heating up or cooling off and why, notable and record SALES, upcoming and just-released SETS and products, rookie call-ups and debuts (huge for cards), injuries or trades that move values, and grading/pop-report news. Only mention an individual performance when you can tie it to a card angle. Prefer 2-4 substantial, explained lines over a long list of results, and NEVER repeat a raw score without a hobby reason.` +
    `\n\n=== COVER EVERY REQUESTED CATEGORY — not just Pokémon ===` +
    `\nA recurring failure is a briefing that is ALL Pokémon and nothing else. That is unacceptable. You MUST produce a section with real, dated content for EVERY category the reader asked for (listed at the end). For a category whose league is off-season or idle, DO NOT skip it — instead fill it from the card hobby, which never stops: recent notable SALES, singles moving in price, upcoming/just-released SETS and products, hot rookies/prospects heating up, grading and pop-report news, and roster/signing news that moves card values. Aim for at least 2 items in every requested category. Pokémon must NOT crowd out the others — give the sports categories equal effort even when there are no games.` +
