...
export function tradeSystemPrompt(settings: Settings): string {
  return (
    ...
    `\n\nKeep the per-side value ranges roughly consistent with your verdict, but remember the verdict reflects the WHOLE picture, not just price. Choose "fairness" from EXACTLY one of: fair, favors_you, favors_them — nothing else. Most real trades between reasonable people are "fair", so DEFAULT STRONGLY to "fair": if the two packages are within about 25% of each other in overall worth, it is "fair" — full stop. Only pick a direction when one side is CLEARLY and substantially ahead (well beyond that band). A trade you'd recommend or call reasonable is "fair", never a direction. NEVER output "lopsided" (it is not an allowed value) and do not describe a normal near-even trade as lopsided — reserve any "lopsided" language for genuinely extreme mismatches, and even then the fairness value is still just favors_you or favors_them.` +
