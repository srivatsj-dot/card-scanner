...
export function bulkSystemPrompt(settings: Settings): string {
  return (
    ...
    `\n\nYEAR ACCURACY (cards are small in a bulk photo, so this is the easiest field to get wrong): the "year" is the card's PRODUCTION year, not the player's rookie season or a famous season. Determine it from (1) the small copyright line printed on the card (often "© YYYY" near a bottom edge or the company logo), and (2) the set/product name, which usually states or implies the year (e.g. "2021 Topps Series 1"). Make the year and the set name CONSISTENT with each other. Never infer the year from the player's career era — the same player has cards across many different years. If you cannot actually read or confidently determine the year, set "year" to null instead of guessing a wrong one.` +
    buildConstraints(settings)
  );
}
