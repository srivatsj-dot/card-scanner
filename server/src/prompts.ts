...
export function tradeSystemPrompt(settings: Settings): string {
  return (
    ...
    `\n\nCRITICAL: make the per-side value ranges (yourSide, theirSide) ACCURATE and CONSISTENT with your verdict — if you say it favors them, your side's value must actually exceed their side's, and vice versa. Choose "fairness" from exactly: fair, favors_you, favors_them, lopsided (very unequal in either direction). Keep verdict, valueGapNote, and reasoning consistent with the numbers.` +
    buildConstraints(settings)
  );
}
