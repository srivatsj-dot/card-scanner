...
export function tradeSystemPrompt(settings: Settings): string {
  return (
    ...
    `\n\nCRITICAL — "valueLow"/"valueHigh" on each side must express that side's OVERALL WORTH TO A COLLECTOR (player caliber and demand included), not merely the raw eBay price. The final verdict is computed from those two numbers, so if you believe one side is clearly better, its range MUST be the higher one. Never write a range that contradicts your own reasoning.` +
    `\n\nAny gap under about 10 ${settings.currency || "USD"} is noise — those trades are ALWAYS fair. Do not split hairs over a few dollars.` +
    ...
    `\n\nKeep the per-side value ranges roughly consistent with your verdict, but remember the verdict reflects the WHOLE picture, not just price. Choose "fairness" from EXACTLY one of: fair, favors_you, favors_them — nothing else. Calibrate it honestly: "fair" when the two packages are genuinely close in overall worth (roughly within 20-25%); pick a DIRECTION whenever one side is clearly ahead. A clear star-for-a-role-player or premium-card-for-a-lesser-one mismatch is NOT fair — e.g. a Shohei Ohtani card for a Ketel Marte card is clearly favors whoever receives Ohtani, never "fair". Do not force "fair" onto an unequal trade just to be agreeable, and do not nitpick a genuinely even trade into a direction over a tiny gap. NEVER output "lopsided" (not an allowed value); an extreme mismatch is still just favors_you or favors_them, with the severity described in the verdict/reasoning.` +
