...
// --- Inter-account trade board (cloud only, auth required) ------------------
async function requireUser(req: Request, res: Response): Promise<number | null> {
  const id = await cloud.userForToken(bearer(req));
  if (!id) { res.status(401).json({ error: "Not signed in." }); return null; }
  return id;
}

app.post("/api/trade/list", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    await cloud.listCard(userId, (req.body as { card?: unknown }).card as Record<string, unknown>);
    res.json({ ok: true });
  } catch (err) { cloudFail(res, err); }
});

app.get("/api/trade/mine", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    res.json({ listings: await cloud.myListings(userId) });
  } catch (err) { cloudFail(res, err); }
});

app.delete("/api/trade/list/:id", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    await cloud.removeListing(userId, Number(req.params.id));
    res.json({ ok: true });
  } catch (err) { cloudFail(res, err); }
});

app.get("/api/trade/board", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    res.json({ listings: await cloud.browseBoard(userId) });
  } catch (err) { cloudFail(res, err); }
});

app.post("/api/trade/offer", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    const { listingId, offered, note } = req.body as { listingId?: number; offered?: Record<string, unknown>[]; note?: string };
    await cloud.makeOffer(userId, Number(listingId), offered || [], note || "");
    res.json({ ok: true });
  } catch (err) { cloudFail(res, err); }
});

app.get("/api/trade/offers", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    res.json(await cloud.getOffers(userId));
  } catch (err) { cloudFail(res, err); }
});

app.get("/api/trade/pending", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    res.json({ pending: await cloud.pendingCount(userId) });
  } catch (err) { cloudFail(res, err); }
});

app.post("/api/trade/offer/:id/respond", async (req: Request, res: Response) => {
  if (!cloudGuard(res)) return;
  try {
    const userId = await requireUser(req, res); if (userId === null) return;
    const accept = !!(req.body as { accept?: boolean }).accept;
    res.json(await cloud.respondOffer(userId, Number(req.params.id), accept));
  } catch (err) { cloudFail(res, err); }
});
