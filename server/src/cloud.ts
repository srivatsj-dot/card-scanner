...
/**
 * Drop briefings generated by an older set of rules. Cached digests are keyed
 * "<generatorVersion>:<date>", so once the rules change every stored briefing
 * from a previous version is stale — including the ones for past days you can
 * page back to. Removing them makes those days regenerate under the new rules
 * instead of serving the old, bad write-up forever.
 */
export async function purgeOldDigests(currentVersion: string): Promise<number> {
  if (!hasCloud) return 0;
  try {
    const r = await db().query(`DELETE FROM digests WHERE key NOT LIKE $1`, [`${currentVersion}:%`]);
    const n = r.rowCount ?? 0;
    if (n) console.log(`[digest] cleared ${n} briefing(s) from older versions — they'll rebuild on demand`);
    return n;
  } catch { return 0; }
}
